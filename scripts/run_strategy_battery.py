...
def main() -> int:
    ...
    for label, params in configs.items():
        ...
        cfg["strategies"][a.strategy].update(params)
        cfg["strategies"][a.strategy]["enabled"] = True
        res = run_bt(cfg, a.strategy, data)
        ...
        m = vo.extract_metrics(res)
        results[label] = {"params": params, "returns": r, "trades": res.trades,
                          "metrics": m, "cpcv": cpcv}
        print(f"[{label:7s}] cpcv={cpcv:+.3f} sharpe={m['sharpe']:+.3f} "
    ...
    print(f"top_ticker {b['top_group_frac']:.2f} | loo_ok {b['loo_group_ok']} | "
          f"min_regime {b['min_regime_sharpe']:.2f} | max_regime_frac {b['max_regime_pnl_frac']:.2f} | "
          f"forward_net {b['forward_net']}")
