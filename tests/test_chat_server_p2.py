...
class TestAlpacaIntradayPnl:
    ...
    def setup_method(self):
        from services.chat_server import _calc_alpaca_intraday_pnl
...
class TestRegimeEndpointStateField:
    """P3.1 — regime/current returns 'state', not 'regime_state'."""

    def test_regime_endpoint_uses_state_field(self):
        """GET /api/regime/current must have 'state' key, not 'regime_state'."""
        ...
        # Import app; override check_auth so we don't need real credentials
        ...
        try:
            ...
            data = resp.json()
            assert "state" in data, f"'state' key missing from {data}"
            assert "regime_state" not in data, (
                f"'regime_state' key must be renamed to 'state', found both: {data}"
            )
        finally:
            app.dependency_overrides.clear()

    def test_regime_endpoint_fallback_uses_state(self):
        """When DB is empty, the fallback dict uses 'state' not 'regime_state'."""
        from fastapi.testclient import TestClient
        from services.chat_server import app, check_auth
        from fastapi.security import HTTPBasicCredentials
        ...
        try:
            ...
            data = resp.json()
            # Either has real data (state key) or fallback {"state": "unknown"}
