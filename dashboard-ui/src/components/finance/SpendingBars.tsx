  ...
  return (
    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="text-[10px] uppercase tracking-wider text-[var(--color-text-muted)] font-medium">SPENDING BY CATEGORY</div>
      ...
      <div>
        {sorted.map((c, i) => (
          <div key={c.category ?? i} className="flex items-center gap-3 mb-2">
            <div className="w-32 text-xs text-[var(--color-text-muted)] truncate">{c.label ?? c.category}</div>
            ...
            <div className="w-20 text-right font-mono text-xs">{fmtCcy(c.amount)}</div>
          </div>
        ))}
      </div>
    </div>
