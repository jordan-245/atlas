...
class CrossSectionalLTReversal(BaseStrategy):
    """Cross-sectional LONG-TERM REVERSAL (DeBondt-Thaler) + low-vol book, long-only. Buys multi-year
    losers (overreaction/value proxy) that are low-vol (avoid value traps). Pass-4 distinct factor."""

    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        c = config.get("strategies", {}).get("cross_sectional_ltreversal", {})
        ...
        self.mom_lookback = int(c.get("mom_lookback", 756))  # ~3 years (LT reversal)
        self.mom_skip = int(c.get("mom_skip", 252))          # skip last ~12 months
        ...
        self.trend_filter = bool(c.get("trend_filter", False))  # counter-trend
    ...
    @property
    def name(self) -> str:
        return "cross_sectional_ltreversal"
    ...
    def precompute(self, data: Dict[str, pd.DataFrame]) -> None:
        ...
        for _ticker, df in data.items():
            ...
            df["_csm_mom"] = -(close.shift(self.mom_skip) / close.shift(self.mom_lookback) - 1.0)  # LT reversal
    ...
    def _factor_row(self, df: pd.DataFrame):
        ...
        if price is None or not np.isfinite(price):
            return None
        # Prefer the precomputed factor (computed on FULL history before the engine windows data to
        # ~train+test bars). For multi-year lookbacks the window is shorter than the lookback, so the
        # raw n-check below would wrongly reject every name -> 0 trades. Precompute is backward-only
        # (shifts/rolling), so using it is look-ahead-free.
        ...
        # Fallback: compute from the tail directly (needs the full lookback in-window).
        n = len(df)
        if n < self.mom_lookback + 2:
            return None
        c = df["close"].to_numpy(dtype=float)
        mom = -(c[-1 - self.mom_skip] / c[-1 - self.mom_lookback] - 1.0)  # LT reversal
