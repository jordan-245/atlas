"""Cross-sectional LOW-VOL + SHORT-REVERSAL book (Pass-3 clean test on survivorship-correct mid/small).\n\nPre-registered from the Pass-2 factor-zoo finding: in mid/small-cap, momentum is dead but low-vol +\nshort-term reversal (counter-trend) are alive. Default = the winning recipe (w_mom=0, w_lowvol=1.0,\nw_rev=0.5, w_hp=0.5, trend_filter OFF). UNBIASED test: --select default + the quarantined 2025-26 holdout.\nOLD multi-factor docstring follows:
...
class CrossSectionalLowvolReversal(BaseStrategy):
    """Configurable cross-sectional multi-factor long book (long-only)."""

    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        c = config.get("strategies", {}).get("cross_sectional_lowvol_reversal", {})
    ...
    @property
    def name(self) -> str:
        return "cross_sectional_lowvol_reversal"
